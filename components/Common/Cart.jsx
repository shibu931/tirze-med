...
const Cart = () => {
    ...
    return (
      <Dialog>
        <DialogTrigger className="hover:bg-fuchsia-100 rounded-full border px-3 py-2 hover:shadow-lg hover:border border-gray-400 hover:border-fuchsia-400 hover:text-fuchsia-800 hover:cursor-pointer relative">
          <BsCart2 className="text-[1.35rem] mt-1" />
          <span className="absolute bg-fuchsia-900 rounded-full text-white font-bold text-xs w-[15px] h-[15px] top-1 -right-1">{cart?.item.length || 0}</span>
        </DialogTrigger>
        <DialogContent>
          <DialogHeader>
        ...
        <DialogFooter className="sm:justify-start">
          <DialogClose asChild>
            <Link
              ...
              className={`text-white hover:bg-fuchsia-900 focus:ring-4 focus:outline-none focus:ring-fuchsia-300 font-medium rounded text-sm px-5 py-2.5 text-center inline-flex w-full justify-center items-center me-2 ${cart?.item.length > 0 ? 'bg-fuchsia-800' : 'bg-gray-400'}`}
            >
              <svg
